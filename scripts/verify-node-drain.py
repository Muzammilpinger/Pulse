...
workers = [
    n
    for n in call("get", "nodes", "-o", "jsonpath={.items[*].metadata.name}").split()
    if "control-plane" not in n and n != postgres_node
]
...
result = {
    ...
    "node": node,
    "postgresNodeExcluded": postgres_node,
}
try:
    ...
    if drain.returncode == 0:
        ...
        result["readyGateways"] = json.loads(
            call("-n", "pulse", "get", "deployment", "gateway", "-o", "json")
        )["status"].get("readyReplicas", 0)
        assert result["readyGateways"] >= 2
finally:
    call("uncordon", node)
result["scope"] = (
    "Voluntary local worker drain with eviction/PDB enforcement. PostgreSQL local-volume node excluded. Not an abrupt node-loss or database-HA test."
)
