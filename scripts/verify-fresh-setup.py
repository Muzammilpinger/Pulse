...
with tempfile.TemporaryDirectory(prefix='pulse-fresh-') as directory:
    ...
    try:
        ...
        env = dict(os.environ, GATEWAY_A='http://localhost:18001', GATEWAY_B='http://localhost:18011', SIGNALING='http://localhost:18002')
        result = subprocess.run(['.venv/bin/pytest', '-q', 'tests'], env=env, text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        Path('docs/evidence/fresh-setup-tests.txt').write_text('Isolated project pulse-fresh-check; new empty PostgreSQL volume.\n' + result.stdout)
