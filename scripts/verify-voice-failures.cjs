...
(async () => {
  ...
  const result = {
    ...
    scope:
      "Local Chromium synthetic microphones; forced relay. No external NAT or audio-quality claim.",
  };
  const pages = [];
  const compose = (...args) =>
  ...
  try {
    ...
    await expect
      .poll(
        () => pages[0].evaluate(() => window.peers.at(-1).connectionState),
        { timeout: 60000 },
      )
      .toMatch(/disconnected|failed|closed/);
    result.turn = {
      secondsUntilLoss: (Date.now() - start) / 1000,
      state: await pages[0].evaluate(() => window.peers.at(-1).connectionState),
      ui: await pages[0].getByRole("status").textContent(),
    };
    compose("start", "coturn");
    for (const p of pages) {
      const leave = p.getByRole("button", { name: "Leave", exact: true });
    ...
    fs.writeFileSync(
      "docs/evidence/voice-failures.json",
      JSON.stringify(result, null, 2) + "\n",
    );
    console.log(result);
  } finally {
    compose("start", "coturn", "signaling");
    await browser.close();
  }
})().catch((e) => {
  console.error(e);
